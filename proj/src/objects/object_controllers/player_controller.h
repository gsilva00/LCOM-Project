 ...
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 * @param player_state_move A pointer to the main player move state, can vary depending on the action chosen, the fase it's in, and the direction it's going.
 * @param player_state_move_temporary A pointer to the auxiliary player move state. Used when an action is requested before the other finishes
 * @param player_state_jump A pointer to the main player jump state, can vary depending on the action chosen, the fase it's in, and the direction it's going.
 * @param player_state_jump_temporary A pointer to the auxiliary player jump state. Used when an action is requested before the other finishes
 * @param player_state_kick A pointer to the main player kick state, can vary depending on the action chosen, the fase it's in, and the direction it's going.
 * @param player_state_kick_temporary A pointer to the auxiliary player kick state. Used when an action is requested before the other finishes
 */
 ...
 * @details The function analyses the player's position, and determines if it is colliding with any border
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 ...
 * @details The function analyses both players positions, and determines if the two items are colliding  
 *
 * @param player1 A pointer to the first player object, of type Player (struct we created).
 * @param player2 A pointer to the second player object, of type Player (struct we created).
 * @return The function returns true if they are colliding, and false if not. In the case of an error it returns 1 as well
 */
 ...
 * @details The function takes the current values of position and speed of the player, and saves them in variables. This function is used in the preparation state of the various player's state machine, so that these values are reset in the end. Auxiliary function of move_player().
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 */
 ...
 * @details The function changes values like the time passed on every axis and the bounce offset value to prepare the player for its jump. This function is used in the preparation state of the player's state machine. Auxiliary function of move_player().
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 */
 ...
 * @details The function changes the player's y value according to the player's current speed values in the y axis, as well as the time spent above the ground. Auxiliary function of handle_player_jump().
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 */
 ...
 * @details The function performs the player's entire jump, from the beginning to the bouncing in the end. It is the function used in the action state of the state machine. Auxiliary function of move_player().
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 * @param player_state_jump A pointer to the player's current jump state, refering to the state machine PlayerStateJump.
 */
 ...
 * @details The function takes the values saved in the beginning by initialize_player_values(), and sets the player's values to those variables, restoring its previous state.
 *
 * @param player A pointer to the player object, of type Player (struct we created).
 */
